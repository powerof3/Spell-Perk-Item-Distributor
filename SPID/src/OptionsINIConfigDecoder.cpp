...
namespace Configs
{
	Config OptionsINIConfigDecoder::decode(const std::filesystem::path& path)
	{
		ini.SetAllowKeyOnly();
		load(path);
		Config config;
	...
	constexpr std::string OptionsINIConfigDecoder::name(const Sections& section)
	{
		using Section = Sections;
		switch (section) {
	...
	bool OptionsINIConfigDecoder::hasOption(const std::string& section, const std::string& name) const
	{
